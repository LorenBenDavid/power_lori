...
final class SupabaseService {
    ...
    func signInWithEmail(email: String, password: String) async throws -> AuthResponse {
        let body: [String: Any] = ["email": email, "password": password]
        return try await post(endpoint: "/auth/v1/token?grant_type=password", body: body, requiresAuth: false)
    }

    func sendPasswordReset(email: String) async throws {
        let body: [String: Any] = ["email": email]
        let _: EmptyResponse = try await post(endpoint: "/auth/v1/recover", body: body, requiresAuth: false)
    }
    ...
    func refreshSession(refreshToken: String) async throws -> AuthResponse {
        ...
        return try await post(endpoint: "/auth/v1/token?grant_type=refresh_token", body: body, requiresAuth: false)
    }
    ...
    func upsertAthleteProfile(_ profile: AthleteProfile, supabaseUserId: String, token: String) async throws {
        let body: [String: Any] = [
        ...
        let _: EmptyResponse = try await post(
            endpoint: "/rest/v1/athlete_profiles",
            body: body,
            token: token,
            method: "POST",
