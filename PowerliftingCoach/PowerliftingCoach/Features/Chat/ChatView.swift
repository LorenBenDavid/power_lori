            ...
            TextField("Ask your coach...", text: $text, axis: .vertical)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 20)
                        .strokeBorder(fieldFocused ? Color.appAccent.opacity(0.6) : Color.white.opacity(0.1), lineWidth: 1)
                )
                .focused($fieldFocused)
                .disabled(isLoading)
                .onSubmit { if !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { onSend() } }
                .onChange(of: isFocused) { _, newValue in fieldFocused = newValue }
                .onChange(of: fieldFocused) { _, newValue in isFocused = newValue }
